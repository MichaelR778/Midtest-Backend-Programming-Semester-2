...
async function getUsers(request, response, next) {
  try {
    ...
    let searchquery = {};
    if (search) {
      searchquery = usersService.getSearchQuery(searchquery, request);
    }

    //sort query
    let sortquery = {};
    if (sort) {
      sortquery = usersService.getSortQuery(sortquery, request);
    } else {
      sortquery.email = 1;
    }

    //convert pagenumber dan pagesize ke integer
    let pagenumber = parseInt(request.query.page_number);
